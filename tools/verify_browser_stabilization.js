...
requireText(controls, 'フォーカス喪失だけでは編集状態を終了しない', 'focus loss must not end edit mode');
requireText(controls, 'focusRequester.requestFocus()', 'address bar focus recovery');
requireText(controls, 'keyboardController?.show()', 'address bar keyboard recovery');
forbidText(controls, 'if (!focus.isFocused && isEditing) onEditingStopped()', 'address focus loss must not end edit mode');
requireText(screen, 'onEditingStopped = viewModel::stopAddressEditing', 'explicit address editing stop wiring');
